...
export interface ElectivePath {
  ...
  depth?: 'survey' | 'deep' | 'specialist'; // survey = overview, deep = applied, specialist = regulatory-grade
  modules: ModuleContent[];
}
    ...
    modules: [
      {
        ...
        estimatedTime: '20 min',
        learningObjectives: [
          'Identify proxy variables in a credit analysis prompt that could correlate with protected characteristics under ECOA (15 U.S.C. §1691)',
          'Apply the three-part disparate impact test (Griggs framework) to evaluate whether an AI prompt creates disproportionate adverse effects on a protected class',
          ...
          'Map CRA assessment area considerations to AI prompt design — ensuring AI-assisted lending analysis does not inadvertently create redlining patterns',
          'Define the escalation trigger for when a fair lending concern requires compliance review before using AI output',
        ],
        learningOutcome: 'After this module, you can audit a credit analysis prompt for fair lending risk, apply the disparate impact framework, and rewrite the prompt to remove proxy variables without losing analytical value.',
        content: {
          overview: 'Fair lending laws — ECOA (15 U.S.C. §1691), Fair Housing Act (42 U.S.C. §3601-3619), CRA (12 U.S.C. §2901) — apply to AI-assisted lending work just as they apply to manual processes. The risk with AI is that prompts or outputs may inadvertently incorporate proxy variables (geography, industry, property type) that correlate with protected classes. ECOA prohibits discrimination on the basis of race, color, religion, national origin, sex, marital status, age, public assistance status, or good faith exercise of rights under the Consumer Credit Protection Act. The Griggs v. Duke Power Co. disparate impact framework (adopted by the CFPB and OCC) means even facially neutral AI prompts can create liability if the output disproportionately affects a protected class and the practice is not justified by a legitimate business necessity. This module teaches you to evaluate AI-assisted lending work for fair lending risks, recognize proxy variable patterns, apply the disparate impact test, and build mitigation into your prompts and review processes. This is not legal advice — it is awareness that helps you know when to escalate to compliance.',
          keyPoints: [
            'ECOA protected classes (Reg B, 12 CFR §1002): race, color, religion, national origin, sex, marital status, age (if applicant has capacity to contract), receipt of public assistance income, good faith exercise of any right under the Consumer Credit Protection Act',
            'Proxy variable awareness: geography (zip codes, census tracts), industry type, property neighborhood, income source (public assistance, alimony), and language preference can serve as proxies for protected characteristics under ECOA and the Fair Housing Act',
            'Disparate impact three-part test (Griggs framework): (1) Does the practice create a disproportionate adverse effect on a protected class? (2) Is the practice justified by a legitimate business necessity? (3) Is there a less discriminatory alternative that serves the same business purpose?',
            'CRA and redlining risk: AI prompts that incorporate geographic weighting, neighborhood characteristics, or "community demographics" may create digital redlining patterns that conflict with CRA assessment area obligations',
            'Prompt audit for fair lending: review any credit analysis prompt to ensure it does not ask the AI to evaluate or weight factors that correlate with protected classes — apply the "would this survive an ECOA examination?" test',
            'Mitigation strategies: (1) remove geographic variables unless operationally necessary and defensible, (2) review AI output for patterns across demographic proxies, (3) document fair lending review for every AI-assisted credit decision, (4) apply the less-discriminatory-alternative test to any factor that could serve as a proxy',
            'Escalation rule: if you suspect an AI output reflects potential bias — geographic concentration, industry-based assumptions, income source weighting — escalate to compliance before using the output. Document the concern, the output, and the escalation per your bank\'s fair lending policy.',
          ],
          practiceTask: {
            title: 'Fair Lending Prompt Audit with Disparate Impact Analysis',
            instructions: 'Evaluate a credit analysis prompt for potential fair lending risks using the ECOA framework and disparate impact test.',
            scenario: 'Review this prompt: "Analyze the risk profile for this commercial loan application. The business is located in the downtown district, serves a primarily Spanish-speaking customer base, and operates in an industry with high employee turnover. Consider location-based risk factors and community demographics in your assessment."\n\nPart 1 — Identify the fair lending risks: name each proxy variable, cite which ECOA protected class it correlates with, and explain the mechanism of correlation.\n\nPart 2 — Apply the Griggs three-part disparate impact test: (a) Does this prompt create a disproportionate adverse effect on a protected class? (b) Is the factor justified by a legitimate business necessity? (c) Is there a less discriminatory alternative?\n\nPart 3 — Rewrite the prompt to eliminate proxy variables while preserving legitimate credit analysis. Annotate each change.\n\nPart 4 — Write your escalation rule: under what specific conditions would you stop using AI output and escalate to compliance? Include what you would document.',
            hints: [
              'Which variables could correlate with protected characteristics under ECOA (15 U.S.C. §1691)?',
              'Apply the three-part disparate impact test — is there a less discriminatory alternative for each factor?',
              'How would you preserve the analytical value while removing bias risk? Focus on credit fundamentals (cash flow, collateral, capacity).',
              'When would you escalate this to compliance? Be specific — what threshold triggers escalation?',
            ],
            successCriteria: [
              'Correctly identifies at least 3 proxy variable risks in the original prompt with ECOA protected class mapping',
              'Applies the Griggs disparate impact three-part test to at least 2 of the proxy variables',
              ...
              'Identifies specific escalation trigger — when to involve compliance, with documentation requirements',
            ],
          },
        },
      },
      {
        ...
        type: 'document',
        description: 'Documenting AI use for examiners — FFIEC guidance, OCC Heightened Standards, and audit trail practices',
        estimatedTime: '20 min',
        learningObjectives: [
          'Create an AI usage log template aligned with FFIEC examination expectations — including column definitions, a realistic example row, and prompt summary guidelines',
          'Prepare confident, specific answers to the 5 examiner questions most frequently asked about AI use, grounded in OCC Heightened Standards (OCC 2014-1) expectations',
          'Map AI documentation practices to the FFIEC IT Examination Handbook requirements for technology governance and change management',
        ...
        content: {
          overview: 'Regulatory examiners are increasingly asking about AI use in banking. The FFIEC IT Examination Handbook establishes expectations for technology governance, and OCC Heightened Standards (OCC Bulletin 2014-1) set the bar for how large and midsize banks manage operational risk — including technology risk. Even community banks that fall below the Heightened Standards threshold face examiner questions framed by these standards. The FDIC\'s Supervisory Guidance on Model Risk Management (SR 11-7 / OCC 2011-12) increasingly applies to AI-assisted outputs that influence credit decisions, risk assessments, or financial reporting. This module covers what to document, how to version-control AI work products, and what examiners will ask — mapped to the regulatory frameworks they\'re using to evaluate your answers.',
          keyPoints: [
            'AI usage log: date, task name, AI tool used, prompt summary (1-2 sentences, not full prompt), output disposition (used as-is / modified / rejected), reviewer name — aligned with FFIEC IT Examination Handbook documentation expectations',
            'FFIEC governance mapping: AI usage documentation maps to the FFIEC IT Handbook sections on IT governance (board and management oversight), change management (new technology adoption), and operational risk (ensuring technology supports safe and sound operations)',
            'OCC Heightened Standards framework (OCC 2014-1): three lines of defense — (1) front-line units own and manage risk, (2) independent risk management provides oversight, (3) internal audit provides assurance. Your AI documentation is Line 1 evidence that risk is being managed.',
            'Model risk management (SR 11-7 / OCC 2011-12): if AI output influences a credit decision, financial projection, or risk rating, examiners may treat it as a "model" requiring documentation of purpose, limitations, validation, and ongoing monitoring',
            ...
            'Examiner questions to prepare for: "What AI tools do you use?", "How do you validate AI output?", "How do you protect customer data?", "Who reviews AI-generated work?", "How do you document AI use?" — your answers should reference your specific log, review process, and data handling protocol',
            'Defensibility standard: could you explain your AI use, data handling, and review process to an examiner without hesitation? If not, improve your documentation. The standard is not perfection — it is transparency and process.',
          ],
          practiceTask: {
            title: 'Create Your AI Usage Log Template (FFIEC-Aligned)',
            instructions: 'Design a department-level AI usage log template aligned with FFIEC IT Examination Handbook expectations and the three lines of defense model.',
            scenario: 'Create an AI usage log template that your department could use starting tomorrow. Include: (1) column definitions with examples — mapped to what an FFIEC examiner would expect to see for technology governance documentation, (2) instructions for what constitutes a "prompt summary" (not the full prompt, but enough context for an auditor), (3) clear disposition categories, (4) a "model risk" flag column for any AI output that influences a credit decision, risk rating, or financial projection (per SR 11-7), (5) a section for periodic review notes. Also prepare brief answers to the 5 examiner questions listed in the key points — frame each answer using the three lines of defense (OCC 2014-1): what YOU do as Line 1, what your compliance team provides as Line 2, and what internal audit checks as Line 3.',
            hints: [
              'Keep columns practical — 5-7 max, but include the model risk flag for SR 11-7 coverage',
              'Show an example row with realistic data — one standard use and one that triggers the model risk flag',
              'Prompt summary should be 1-2 sentences, not the full prompt',
              'Frame your examiner answers with the three lines of defense — what does each line own?',
            ],
            successCriteria: [
              'Log template has clear column definitions with examples aligned to FFIEC IT governance expectations',
              ...
              'All 5 examiner questions have prepared, confident responses framed with the three lines of defense',
            ],
          },
        },
      },
      {
        ...
        type: 'exercise',
        description: 'Evaluating third-party AI tools using OCC 2023-17 Third-Party Risk Management and interagency guidance',
        estimatedTime: '20 min',
        learningObjectives: [
          'Apply the OCC Third-Party Risk Management guidance (OCC 2023-17) life cycle framework — planning, due diligence, contract negotiation, ongoing monitoring, and termination — to an AI vendor assessment',
          ...
          'Identify at least 3 specific contract provisions to negotiate in any AI vendor agreement, aligned with the Interagency Guidance on Third-Party Relationships (June 2023)',
          'Produce an approve, approve-with-conditions, or reject recommendation based on documented assessment results with risk-commensurate due diligence',
        ],
        learningOutcome: 'After this module, you can complete a structured vendor AI risk assessment aligned with OCC 2023-17 and deliver an evidence-based approve/reject recommendation.',
        content: {
          overview: 'Your bank will evaluate AI vendors — and your input as a user matters. The OCC\'s Third-Party Risk Management guidance (OCC Bulletin 2023-17, June 2023) and the Interagency Guidance on Third-Party Relationships (OCC/FDIC/Federal Reserve, June 2023) establish the framework for how banks must assess, contract with, monitor, and exit third-party technology relationships. The FDIC\'s Technology Service Provider guidance (FIL-44-2008) adds specific expectations for technology vendors serving regulated institutions. For AI vendors specifically, the guidance requires risk-commensurate due diligence — meaning the depth of your assessment should match the criticality of the vendor\'s role. An AI tool used for customer-facing credit decisions requires deeper assessment than one used for internal draft generation. This module gives you a structured assessment framework covering the five areas that matter most, mapped to the regulatory lifecycle.',
          keyPoints: [
            'OCC 2023-17 life cycle: (1) Planning — define the business need, assess whether the activity is "critical" (customer-facing, significant financial impact, or compliance-critical), (2) Due diligence — assess the vendor\'s financial condition, expertise, technology, controls, and compliance posture, (3) Contract negotiation — include all required provisions, (4) Ongoing monitoring — review performance, financials, and controls regularly, (5) Termination — plan for data return/destruction and transition',
            'Critical activity designation: if the AI vendor touches customer data, influences credit decisions, or generates examiner-facing documentation, it is likely a "critical activity" requiring enhanced due diligence under OCC 2023-17',
            'Data handling: Where is data stored? Is it used for model training? What is the retention period? Can data be deleted on request? Is encryption in transit and at rest? The Interagency Guidance requires banks to understand the vendor\'s data practices and subprocessor chain',
            'Security certifications: SOC 2 Type II (required), penetration testing (required), BAA for PII handling (required if applicable), FedRAMP (preferred for cloud). FDIC FIL-44-2008 requires that banks ensure technology service providers maintain adequate security controls.',
            'Contract provisions (Interagency Guidance, Section V): data deletion upon termination, breach notification timeline (72 hours max), liability for AI errors, right to audit, subprocessor disclosure, business continuity and disaster recovery provisions, regulatory examination access, and default and termination rights',
            'Regulatory support: can the vendor provide documentation for examiners? Do they support regulatory examination access (required by OCC 2023-17)? Have they passed FFIEC-based evaluations? Will they cooperate with your bank\'s internal audit of the relationship?',
            'Practical evaluation: test with YOUR banking scenarios, not the vendor\'s demos. A tool that demos well may underperform on your specific document types, compliance language, or formatting requirements. Document test results as evidence of your due diligence process.',
          ],
          practiceTask: {
            title: 'Complete a Vendor AI Risk Assessment (OCC 2023-17 Aligned)',
            instructions: 'Complete a vendor risk assessment using the OCC Third-Party Risk Management life cycle framework.',
            scenario: 'Choose an AI tool (real or hypothetical). Complete a risk assessment following the OCC 2023-17 life cycle:\n\n(1) Planning — Is this a "critical activity"? Justify your designation based on whether the tool touches customer data, influences credit decisions, or generates examiner-facing documentation.\n(2) Due diligence — Data handling (5 questions with answers or "need to research"), Security certifications (checklist with pass/fail/unknown), Vendor financial stability and operational resilience.\n(3) Contract negotiation — 5 critical terms to negotiate, aligned with the Interagency Guidance on Third-Party Relationships (June 2023), including regulatory examination access and data deletion upon termination.\n(4) Ongoing monitoring — How will you monitor vendor performance, security posture, and financial condition on an ongoing basis?\n(5) Termination planning — What happens to your data if you exit the relationship? How will you transition?\n\nAlso include a practical evaluation: results of testing with 3 banking scenarios. Conclude with a recommendation: approve, approve with conditions, or reject — with risk-commensurate rationale.',
            hints: [
              'Start with the "critical activity" designation — it determines the depth of everything else',
              'Be honest about what you know vs what needs research — examiners respect "we identified this gap and are addressing it"',
              'Name specific contract terms from the Interagency Guidance — not vague "good contract"',
              'Test with real banking tasks, not generic prompts — document results as due diligence evidence',
            ],
            successCriteria: [
              'All 5 OCC 2023-17 life cycle phases addressed with specific content',
              'Critical activity designation is justified with specific criteria',
              'Data handling questions are specific and answered honestly',
              'Security checklist distinguishes between pass/fail/unknown',
              'At least 3 specific contract provisions named with reasoning aligned to Interagency Guidance',
              'Practical evaluation uses banking-specific test scenarios',
              'Recommendation is clear with risk-commensurate supporting rationale',
            ],
          },
        },
      },
