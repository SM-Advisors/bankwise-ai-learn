...
export function ProficiencyAssessment({ onComplete }: ProficiencyAssessmentProps) {
  ...
  const totalSteps = PROFICIENCY_QUESTIONS.length + 1; // questions + confidence
  const isOnConfidence = currentQuestion >= PROFICIENCY_QUESTIONS.length;
  const isComplete = showResult;

  const currentQ = PROFICIENCY_QUESTIONS[currentQuestion];
  const progressPercent = ((currentQuestion + (isOnConfidence ? 1 : 0)) / totalSteps) * 100;
  ...
  const handleNext = () => {
    if (!isOnConfidence && currentQ && answers[currentQ.id] === undefined) return;
    if (isOnConfidence && confidence === null) return;

    if (isOnConfidence) {
      // Show result
      setShowResult(true);
    } else if (currentQuestion < PROFICIENCY_QUESTIONS.length - 1) {
      setCurrentQuestion((prev) => prev + 1);
    } else {
      // Move to confidence step
      setCurrentQuestion(PROFICIENCY_QUESTIONS.length);
    }
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between text-xs text-muted-foreground mb-1">
        <span>Question {currentQuestion + 1} of {totalSteps}</span>
        <span>{Math.round(progressPercent)}%</span>
      </div>
      <Progress value={progressPercent} className="h-1.5" />
      ...
      <div className="flex justify-between">
        <Button variant="outline" onClick={handleBack} disabled={currentQuestion === 0} className="gap-2">
          <ArrowLeft className="h-4 w-4" />
          Back
        </Button>
