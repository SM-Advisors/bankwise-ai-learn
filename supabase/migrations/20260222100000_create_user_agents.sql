...
CREATE INDEX idx_user_agents_deployed ON user_agents(user_id, is_deployed) WHERE is_deployed = true;
CREATE INDEX idx_user_agents_shared ON user_agents(is_shared) WHERE is_shared = true;
...
CREATE INDEX idx_agent_test_conversations ON agent_test_conversations(agent_id, test_type);

ALTER TABLE agent_test_conversations ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Users can manage own test conversations"
  ON agent_test_conversations FOR ALL
  USING (auth.uid() = user_id);
