...
CREATE POLICY "Users can read own retrieval responses"
  ON retrieval_responses FOR SELECT
  USING (auth.uid() = user_id);

CREATE POLICY "Users can insert own retrieval responses"
  ON retrieval_responses FOR INSERT
  WITH CHECK (auth.uid() = user_id);

-- Allow service role (edge functions) to insert on behalf of users
CREATE POLICY "Service role can manage retrieval responses"
