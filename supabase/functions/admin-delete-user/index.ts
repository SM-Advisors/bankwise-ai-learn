...
Deno.serve(async (req) => {
  ...
  try {
    ...
    await adminClient.from("dashboard_conversations" as never).delete().eq("user_id", user_id);
    await adminClient.from("agent_test_conversations" as never).delete().eq("user_id", user_id);
    await adminClient.from("user_agents").delete().eq("user_id", user_id);
    await adminClient.from("user_ideas" as never).delete().eq("user_id", user_id);
    ...
    await adminClient.from("skill_observations" as never).delete().eq("user_id", user_id);
    await adminClient.from("response_feedback" as never).delete().eq("user_id", user_id);
    await adminClient.from("submission_scores" as never).delete().eq("user_id", user_id);
    await adminClient.from("retrieval_responses" as never).delete().eq("user_id", user_id);
    await adminClient.from("rate_limit_events" as never).delete().eq("user_id", user_id);
