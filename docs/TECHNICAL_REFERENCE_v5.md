...
**Change Summary (v4.0 → v5.0):**
- **Curriculum v2.0 rewrite** (Rivers Meeting, March 7 2026): 5 sessions with 27 modules replace the prior 4 sessions / 22 core + 12 elective structure. Conversation-first, confidence-before-complexity philosophy throughout
- **Session 5 — Build Your Frankenstein**: New capstone session where learners design their own AI stack
- **Elective paths removed**: ELECTIVE_PATHS array is now empty; elective concepts folded into Sessions 4 and 5
- **Agents zone** (6th learner zone): Persistent agent chat unlocked after first agent deployment; `session_3_agent_deployed` unlock condition
- **Agent sharing**: `is_shared` / `shared_at` on `user_agents`; `SharedAgentChat` page at `/agent/:agentId`; `ShareDialog` component for multi-destination sharing (community, ideas, executive)
- **Agent knowledge base & tools**: `AgentTemplateData` now includes `conversation_starters`, `knowledge_base`, `enabled_tools`; `extract-agent-knowledge` edge function parses PDF/DOCX/TXT/CSV for Agent Studio
- **Voice-to-text**: `speech-to-text` edge function (Deepgram nova-2); `useVoiceToText` hook for audio recording and transcription
- **Idea preview generation**: `generate-idea-preview` edge function (Gemini) creates self-contained HTML prototypes
- **Model upgrade**: Primary LLM upgraded from Claude Sonnet to **Claude Opus 4.6** for trainer_chat, submission_review, practice_chat, agent-test-chat, workflow-test-chat
- **Admin Andrea sidebar**: Andrea advisor panel is now a sticky right sidebar visible on ALL admin tabs (not just C-Suite)
- **Admin Andrea persistence**: New `admin_andrea_notes` and `admin_andrea_conversations` tables store admin Andrea chat and notes
- **Organization platform selector**: `organizations.platform` column (`default` | `chatgpt`) enables platform-specific UI rendering
- **Zone unlock changes**: Explore zone now gated by module 1-3 (`session_1_basic_interaction_done`) instead of module 1-2
- **Profile hub restructured**: Tabs are now Personalization, Ideas & Feedback, Journey (was Settings, Memories, Journey)
- **Retake intake form**: Users can retake the onboarding intake from their Profile dropdown
- **Rate limiter formalized**: Per-function rate limits documented in `_shared/rateLimiter.ts` with specific per-minute and per-day caps
- **Session 5 progress tracking**: `session_5_completed` and `session_5_progress` columns on `training_progress`; `current_session` constraint extended to 1–5
- **Registration code RPC**: `validate_registration_code()` rewritten as proper Supabase RPC function
- **Edge function count**: 18 (up from 15): added `speech-to-text`, `extract-agent-knowledge`, `generate-idea-preview`
- **Total migrations**: 61 (up from 54)
...
|---------|---------|-------|
| **Primary LLM** | Anthropic Claude Opus 4.6 | Andrea coaching, submission review, practice AI, agent testing, workflow testing |
| **Embeddings** | OpenAI text-embedding-3-small | RAG chunk embeddings (1536 dimensions) |
| **Lesson Generation** | Google Gemini (via Lovable AI Gateway) | AI-powered lesson generation |
| **Document Parsing** | Google Gemini (gemini-2.5-flash + lite) | PDF/DOCX policy document extraction, agent knowledge base extraction |
...
│Claude Opus4.6│ │text-embed-3 │ │(Gemini mdls) │ │  nova-2  │
└──────────────┘ └─────────────┘ └──────────────┘ └──────────┘
```
...
**Current edge functions (18 total):**
`trainer_chat`, `submission_review`, `ai-practice`, `agent-test-chat`, `workflow-test-chat`, `practice_chat`, `ai-trainer`, `generate-lesson`, `embed_chunks`, `seed_lesson_chunks`, `parse-policy-document`, `intake-prompt-score`, `admin-create-user`, `admin-delete-user`, `speech-to-text`, `extract-agent-knowledge`, `generate-idea-preview`, `_shared` (utilities)
...
- Injects the complete curriculum module map (27 modules across 5 sessions)
...
**Curriculum v2.0** (Rivers Meeting, March 7, 2026). Defines all 5 sessions and 27 modules. Governing principles: everything works, confidence before complexity, conversation-first computing, role-specific from the start.
...
1. Training content defined in `trainingContent.ts` (27 modules across 5 sessions)
...
|----------|-------|-----------|-------|
| `trainer_chat` (Andrea) | Claude Opus 4.6 | 1000 | 400 for greeting mode; curriculum v2.0 with 5-session navigation map |
| `submission_review` | Claude Opus 4.6 | 1200 | Learning-style-adaptive feedback; proficiency-scaled guidance (0-8) |
| `ai-practice` | Claude Opus 4.6 | 1500 | Mirror prompt quality; brainstorm persona; agent system prompt mode |
| `agent-test-chat` | Claude Opus 4.6 | 1500 | Wrapped system prompt |
| `workflow-test-chat` | Claude Opus 4.6 | 1500 | Step-specific context |
...
Organizations can configure `allowed_models` to control which models are available to their learners. Users' preferred model is persisted in `user_profiles.preferred_model`.
...
**Curriculum v2.0** — Rewritten from the ground up based on the Rivers Meeting (March 7, 2026). The curriculum now consists of **5 sessions with 27 modules**. The prior 4-session / 22-module + 12-elective structure has been replaced entirely.
...
### 11.2 Session 1: Foundation & Early Wins (3 Modules)
...
### 11.3 Session 2: Structured Interaction, Models & Tools (7 Modules)

Andrea Tier: **Collaborative** — ask before telling, push specificity.

| Module | Title | Type | Est. Time | Key Content |
|--------|-------|------|-----------|-------------|
| 2-1 | Structured Prompting (CLEAR Framework) | document | 15 min | Context, Length, Examples, Audience, Requirements framework |
...
| 2-7 | Sandbox | sandbox | 15 min | Open-ended exploration and practice |
...
- **Training Modules:** View all 27 module definitions across 5 sessions. Trigger `seedAllContent()` for combined seeding and embedding.
...
- **Model Configuration:** Configure `allowed_models` per organization to control available AI models.
...
All migrations use `IF NOT EXISTS` for idempotency. **Total: 61 migrations.**
...
*End of SMILE Comprehensive Technical Reference v5.0*
