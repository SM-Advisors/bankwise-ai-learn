...
- New AppShell navigation system: `AppShell`, `NavRail`, `TopBar`, `AndreaDock` components replace legacy layout
- Zone-based progressive disclosure: 5 learner zones (Home, Learn, Explore, Community, Profile) with unlock conditions via `useFeatureGates`
- SMILE design system primitives: `SmileCard`, `ProgressStrip`, `EmptyState`, `SkeletonLoader`
- Route restructuring: Profile hub absorbs Settings/Memories/Journey; Explore zone hub; Community zone; full-screen training workspaces
- Dashboard redesigned as state machine: `brand_new` | `mid_session` | `between_sessions` | `all_complete`
- TrainingWorkspace: inline ModuleContentPanel (65%), SessionSwitcher bar, quality-gated module completion
- Onboarding redesigned to 5-step flow: Display Name → Job Role → Prompt Assessment → Learning Style → AI Preferences
- Proactive Andrea trigger engine (`useAndreaTriggers`): return engagement, feature unlock, stall detection
- BrainstormPanel: Andrea-guided workflow discovery with submit-to-Ideas/Community/C-Suite
- ExecutiveOverview: C-suite KPI attention items with severity badges and funnel chart
- 12 new database tables: `industries`, `departments`, `roles`, `executive_submissions`, `skill_observations`, `level_change_requests`, `rate_limit_events`, `retrieval_responses`, `response_feedback`, `submission_scores`, `certificates`, `proficiency_responses`
- Schema changes: `user_profiles` gains `is_active`, `is_super_admin`, `interests[]`, `intake_responses`, `safe_use_flag`, `intake_role_key`, `intake_orientation`, `intake_motivation[]`, `preferred_model`; `organizations` gains `allowed_models`, `org_type`; `community_topics` gains review queue (`status`, `category`, `source_type`); `user_ideas` gains `source`, `submitted_to_exec`, preview columns
- Data retention cron jobs via pg_cron (12-month conversations, 30-day rate limits)
- Value signals analytics (`useValueSignals`) for impact tracking
- Design system: Inter + Playfair Display fonts, navy/orange palette, dark mode support
- New packages: `zod`, `remark-gfm`, `@tailwindcss/typography`, `vitest`
- Total migrations: 54 (up from 51)
...
- **Progressive disclosure**: The platform reveals features (Explore, Community, Profile zones) as learners progress through training — no overwhelming information dump upfront.
...
- **Quality-gated progression**: Module completion requires passing a quality gate on practice submissions — learners can't skip ahead without demonstrating competence.
...
Organizations can be typed as `bank` (default) or `friends_family` (pilot testers). Each organization can configure `allowed_models` to control which AI models are available to their learners in practice sessions.
...
### 3.2 AppShell Navigation Architecture (v4 new)
...
### 3.3 Zone-Based Navigation (v4 new)
...
│ Explore     │ /explore      │ session_1_module_2_done (CLEAR)│
...
### 4.6 Proactive Andrea Trigger Flow (v4 new)
...
### 4.7 Zone Progressive Disclosure Flow (v4 new)
...
│ ├── 'session_1_module_2_done'                      │
│ │   → session_1_progress.completedModules          │
│ │     .includes('1-2')  (CLEAR Framework)         │
│ ├── 'first_practice_done'                          │
...
| `is_active` | boolean | **v4** — Soft deactivation flag (default true) |
| `is_super_admin` | boolean | **v4** — Super admin flag (default false) |
| `interests` | text[] | **v4** — Interest tags for F&F (non-banker) users |
| `intake_responses` | jsonb | **v4** — Full intake questionnaire answers |
| `safe_use_flag` | boolean | **v4** — Triggered if user indicated they'd paste PII into external AI |
| `intake_role_key` | text | **v4** — Role key from intake (e.g., 'loan_officer', 'compliance') |
| `intake_orientation` | text | **v4** — AI orientation feeling (excited/curious/anxious/skeptical/neutral) |
| `intake_motivation` | text[] | **v4** — Motivation tags from intake multi-select |
| `preferred_model` | text | **v4** — Last-selected AI model in practice chat (default 'claude-sonnet-4-6') |
...
| `org_type` | text | **v4** — `bank` (default) or `friends_family` (F&F pilot testers) |
| `allowed_models` | jsonb | **v4** — Array of allowed model IDs (default `["claude-sonnet-4-6"]`) |
| `created_at` | timestamptz | Created |
| `updated_at` | timestamptz | Updated |
...
| `category` | text | **v4** — discussion / idea / friction_point / shared_agent / shared_workflow |
| `source_type` | text | **v4** — manual / andrea_suggested / andrea_user_requested |
| `linked_content_id` | uuid | **v4** — FK to linked agent/workflow |
| `linked_content_type` | text | **v4** — agent / workflow / NULL |
| `status` | text | **v4** — pending / approved / rejected (review queue) |
| `created_at` | timestamptz | Created |
| `updated_at` | timestamptz | Updated |
...
| `source` | text | **v4** — manual / andrea_suggested / andrea_user_requested |
| `source_context` | text | **v4** — Module/context where idea originated |
| `linked_agent_id` | uuid | **v4** — FK → user_agents if idea linked to agent |
| `submitted_to_exec` | boolean | **v4** — Whether escalated to executive review |
| `preview_html` | text | **v4** — Generated HTML prototype preview |
| `preview_status` | text | **v4** — none / generating / generated / failed |
| `preview_generated_at` | timestamptz | **v4** — When preview was generated |
| `created_at` | timestamptz | Created |
| `updated_at` | timestamptz | Updated |
...
**Seed data:** `banking` / `Community Banking`

### 5.27 Table: `departments` *(v4 new)*
...
### 5.29 Table: `executive_submissions` *(v4 new)*
...
### 5.30 Table: `skill_observations` *(v4 new)*
...
### 5.31 Table: `level_change_requests` *(v4 new)*
...
### 5.32 Table: `rate_limit_events` *(v4 new)*
...
### 5.33 Table: `retrieval_responses` *(v4 new)*
...
### 5.34 Table: `response_feedback` *(v4 new)*
...
### 5.35 Table: `submission_scores` *(v4 new)*
...
### 5.36 Table: `certificates` *(v4 new)*
...
### 5.37 Table: `proficiency_responses` *(v4 new)*
...
**v4 note:** Submission scores are now persisted to `submission_scores` table for analytics and quality gate enforcement.
...
**Purpose:** Simulates a real AI tool for learners to practice prompting against. No coaching. Also used by BrainstormPanel with custom brainstorm persona.
...
### 7.14 Proactive Trigger System (v4 new)
...
Protected Routes — Profile Zone Hub:
  /profile            → Profile.tsx (3-tab hub: Settings, Memories, Journey)

Profile Zone Redirects:
  /settings           → Redirects to /profile?tab=settings
  /memories           → Redirects to /profile?tab=memories
...
### 8.3 AppShell Architecture (v4 new)
...
### 8.6 AndreaDock Component (v4 new)
...
### 8.7 SMILE Design Primitives (v4 new)
...
### 8.8 Zone Configuration (v4 new)

**File:** `src/config/zones.ts`

```typescript
export const LEARNER_ZONES: Zone[] = [
  { id: 'home',      icon: Home,     label: 'Home',      path: '/dashboard',   unlockedBy: 'always' },
  { id: 'learn',     icon: BookOpen, label: 'Learn',     path: '/training/1',  unlockedBy: 'onboarding_completed' },
  { id: 'explore',   icon: Compass,  label: 'Explore',   path: '/explore',     unlockedBy: 'session_1_module_2_done' },
  { id: 'community', icon: Users,    label: 'Community', path: '/community',   unlockedBy: 'first_practice_done' },
  { id: 'profile',   icon: User,     label: 'Profile',   path: '/profile',     unlockedBy: 'onboarding_completed' },
];
```
...
| `all_complete` | All 4 sessions finished | Congratulations + top skills list + Explore Electives + Community CTAs |
...
Full-screen training interface (no AppShell). Three-panel layout:

- **SessionSwitcher** (top): 4 session pills with state indicators (active/completed/accessible/locked)
- **Left panel (65%):** `ModuleContentPanel` — inline module viewer with scroll progress, learning outcomes, key points, examples, steps
- **Center panel (35%):** `PracticeChatPanel` — AI practice conversation interface
...
#### `Profile.tsx` (`src/pages/Profile.tsx`) *(v4 new)*

3-tab user hub (replaces separate Settings, AIMemories, AIJourney pages):
- **Settings tab**: AI preferences (tone/verbosity/format), role context, custom instructions
- **Memories tab**: AI memories (pinned/unpinned), create/delete, source tracking
- **Journey tab**: Skill progression chart, session progress bars, certificate list

Tab navigation via URL query: `/profile?tab=settings|memories|journey`

#### `Explore.tsx` (`src/pages/Explore.tsx`) *(v4 new)*

Feature hub landing page (unlocked after CLEAR Framework in module 1-2). Grid of 5 feature cards:
- Prompt Library (/prompts)
- My Ideas (/ideas)
- Elective Paths (/electives) — locked until Session 3 complete
- My AI Journey (/journey)
- Certificates (/certificates)

#### `CommunityZone.tsx` (`src/pages/CommunityZone.tsx`) *(v4 new)*
...
Multi-tab admin panel with **flex-wrap** tab layout.

| Tab | Content |
|-----|---------|
| Overview | ExecutiveOverview (v4) — KPI snapshot with attention items + funnel chart |
...
| Reporting | Engagement analytics, session completion rates; CSV export |
...
#### `ModuleContentPanel.tsx` (`src/components/training/ModuleContentPanel.tsx`) *(v4 new)*
...
- Top bar: New Chat + Model selector + History drawer
...
#### `SessionSwitcher.tsx` (`src/components/training/SessionSwitcher.tsx`) *(v4 new)*

4-session pill navigation bar:
...
#### `BrainstormPanel.tsx` (`src/components/BrainstormPanel.tsx`) *(v4 new)*
...
#### `ExecutiveOverview.tsx` (`src/components/admin/ExecutiveOverview.tsx`) *(v4 new)*
...
#### `useFeatureGates` (`src/hooks/useFeatureGates.ts`) *(v4 new)*
...
Reads from `useAuth()` without additional DB queries.

#### `useValueSignals` (`src/hooks/useValueSignals.ts`) *(v4 new)*
...
#### `useAndreaTriggers` (`src/hooks/useAndreaTriggers.ts`) *(v4 new)*
...
Defines all 4 sessions and 22 core modules, plus exports `ELECTIVE_PATHS` containing 4 elective learning paths with 3 modules each. Each module contains: id, title, description, keyPoints, examples, steps, practiceTask, successCriteria.
...
#### `src/utils/andreaTriggerStorage.ts` *(v4 new)*
...
**v4 update:** `validate_registration_code()` now returns `org_type` from the organization record, allowing the frontend to tailor the onboarding experience for F&F (friends & family) users vs. bank employees.

### 9.6 User Deactivation (v4 new)
...
| `submission_review` | claude-sonnet-4-20250514 | 1200 | Structured JSON; scores persisted to `submission_scores` |
| `ai-practice` | claude-sonnet-4-20250514 | 1500 | Mirror prompt quality; brainstorm persona |
...
**v4 note:** Organizations can configure `allowed_models` to control which models are available to their learners. Users' preferred model is persisted in `user_profiles.preferred_model`.
...
### 10.9 Skill Observations (v4 new)
...
**v4 note:** Completing module 1-2 unlocks the Explore zone.
...
**v4 redesigned:** The assessment is now part of the 5-step onboarding flow (Step 3). Scenario-based questions with scoring mapped to proficiency levels. Full responses stored in `proficiency_responses` table for audit trail.

**Scoring:** Composite score normalized to 0-8 integer. Stored as `user_profiles.ai_proficiency_level`.

### 11.8 Quality-Gated Progression (v4 new)

Module completion requires passing a quality gate:
...
- **Training Modules:** View all 22 core module definitions + 12 elective modules. Trigger `seedAllContent()` for combined seeding and embedding.
...
- **Organizations:** Create org records (name + slug + audience_type + industry + org_type + allowed_models). View all orgs with user counts.
- **Registration Codes:** Create invitation codes with optional max_uses and expires_at. Deactivate codes.
- **Model Configuration (v4):** Configure `allowed_models` per organization to control available AI models.
...
- **Executive Overview (v4):** New admin landing with KPI attention items, severity badges, and funnel chart.
- **Engagement Reporting:** Message volume over time, active user counts, module completion rates. CSV export.
...
- **Submission Scores (v4):** View rubric scores per learner/module for quality gate analytics.

### 12.6 Community Moderation (v4 new)
...
7. **Community Hub Content Moderation** — User-generated content in community feeds. **v4 update:** Now mitigated by pending/approved review queue.
8. **Admin Email Hardcoding** — Break-glass admin list is hardcoded in frontend source. Risk accepted as intentional fail-safe.
9. **Data Retention** — Conversation data accumulates indefinitely. **v4 update:** Now mitigated by pg_cron retention jobs (12 months for conversations, 30 days for rate limits).
...
11. **Rate Limiting Bypass** — If `rate_limit_events` table is inaccessible, rate limiting fails open. Recommendation: add circuit breaker to edge functions.
...
14. **Preview HTML Generation** — User ideas can generate HTML previews. Recommendation: sanitize preview_html before rendering to prevent XSS.
...
All migrations use `IF NOT EXISTS` for idempotency. **Total: 54 migrations.**
...
| `20260222300000_create_departments_system.sql` | **v4** — industries, departments, roles tables with RLS and seed data |
| `20260222400000_sharing_infrastructure.sql` | **v4** — executive_submissions table; extends community_topics and user_ideas with category/source/linking columns |
| `20260222500000_skill_assessment.sql` | **v4** — skill_observations, level_change_requests tables |
| `20260222700000_smile_v2_tables.sql` | **v4** — user_prompts, elective_progress, certificates, proficiency_responses tables; session_4 columns on training_progress |
...
| `20260223100000_org_model_settings.sql` | **v4** — allowed_models on organizations; preferred_model on user_profiles |
| `20260223100001_rate_limiting.sql` | **v4** — rate_limit_events table (service role only) |
| `20260223150833_*.sql` | Misc patches |
| `20260223152151_*.sql` | Misc patches |
| `20260223200000_data_retention_cron.sql` | **v4** — pg_cron jobs for 12-month conversation retention, 30-day rate limit cleanup |
| `20260223200001_retrieval_responses.sql` | **v4** — retrieval_responses table for spaced repetition |
| `20260223204014_*.sql` | Misc patches |
| `20260223300000_community_review_queue.sql` | **v4** — Adds status column to community_topics; review queue RLS policy |
| `20260223400000_user_deactivation.sql` | **v4** — is_active flag on user_profiles |
| `20260223500000_response_feedback.sql` | **v4** — response_feedback table (thumbs up/down) |
| `20260223500001_super_admin_and_ff.sql` | **v4** — is_super_admin on user_profiles; org_type on organizations; interests[] for F&F; updated validate_registration_code() |
| `20260223600000_intake_revamp.sql` | **v4** — intake_responses, safe_use_flag, intake_role_key, intake_orientation, intake_motivation[] on user_profiles |
| `20260223600001_submission_scores.sql` | **v4** — submission_scores table for persisted rubric scores |
...
| `20260227000000_idea_code_preview.sql` | **v4** — preview_html, preview_status, preview_generated_at on user_ideas and executive_submissions |
| `20260228165218_*.sql` | Cleanup: reset stale preview_status |

---

*End of SMILE Comprehensive Technical Reference v4.0*
